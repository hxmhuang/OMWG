...
ls ${MSPM}/${SEASCLIM}
if ($status == 0) then
 echo "reading ${SEASCLIM} from archive"
 cp ${MSPM}/${SEASCLIM} .
 if ($doall == 0) ncks -O -h -v ${varstr} ${SEASCLIM} ${SEASCLIM}
  ...
  while ($yr <= $yr1)
    ...
    #cp ${MSP}/${CASENAME}.pop.h.${year}-{01,02,03,04,05,06,07,08,09,10,11,12}.nc . & 
      ...
      set filesize =  `ls -l ${MSP}/${CASENAME}.pop.h.${year}-01.nc | awk '{print $5}'`
    endif

    if ($doall == 0) then
      foreach mon (01 02 03 04 05 06 07 08 09 10 11 12)
          ncks -O -h -v ${varstr} ${MSP}/${CASENAME}.pop.h.${year}-${mon}.nc  \
  ...
  if ($yr1 == $yr0) then
    ncrcat ${MSP}/${CASENAME}.pop.h.${yr0}-??.nc $SEASCLIM
  else

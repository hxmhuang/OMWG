module POP_grid_mod
  ...
  subroutine read_POP_grid_NetCDF(filename, grid_var)
     ...
     character(len=char_len), intent(in) :: &
          filename           ! file containing grid vars
     ...
     call check_var_size(ncid, KMT_name, 2, (/ grid_var%nlon, grid_var%nlat /))
     ...
     call nf_inq_varid_wrap(ncid, KMT_name, varid, subname)
     call nf_get_var_wrap(ncid, varid, grid_var%kmt, subname)

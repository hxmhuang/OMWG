...
END {
	...
	>> "diagts_cfc.asc"
     ...
     printf("%10s%3s%3s%5s%15s%15s%15s%15s%14s%14s%14s%14s\n","Time Stamp","M","D","Y", \
        "diatChl","spChl","diazChl","Fe","NO3","PO4","SiO3","O2") > "diagts_ecosys.asc"
     for (x = 1; x <= globdiagcnt; x++) {
       y = globdiagmmddyyyy[x]
       printf("%10s%3i%3i%5i%2s%+.6E%2s%+.6E%2s%+.6E%2s%+.6E%14.7f%14.7f%14.7f%14.7f\n",globdiagdate[y],globdiagmonth[y], \
        globdiagday[y],globdiagyear[y],"  ",diatchl[y],"  ",spchl[y],"  ",diazchl[y],"  ",fe[y],no3[y],po4[y],sio3[y],o2[y]) \
        >> "diagts_ecosys.asc"
     }

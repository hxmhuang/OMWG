...
if ($NEED_CLIM == 1) then
 ...
 echo 'i='$i
   ...
   ls ${MSPM}/${SEASCLIM}
   if ( $status == 0 ) then
     echo "File ${MSPM}/${SEASCLIM} already exists...reading from archive"
     ln -sf ${MSPM}/${SEASCLIM} ${SEASCLIM} 
   else
     @ count++
       if ($count == 1) then
         set varstr = ${i}
 ...
 if ($count == 0) then
   echo "all requested climatologies exist and have been read from archive"
   setenv NEED_CLIM 0
 endif
endif
...
while ($yr <= $yr1) 
  ...
  if (($NEEDTAVG == 1 && $CREATE == 1) || $NEED_CLIM == 1 ) then
   ...
   if ($status == 0) then
     ...
     #cp ${MSP}/${CASENAME}.pop.h.${year}-{01,02,03,04,05,06,07,08,09,10,11,12}.nc . &
     #wait
   else
     ls ${MSP}/${CASENAME}.pop.h.${year}.tar
     if ($status == 0) then
       ...
       set sizes = `ls -l ${MSP}/${CASENAME}.pop.h.${year}-??.nc | awk '{print $5}'`
       if ( $#sizes == 12 && \
         $sizes[1] == $filesize && \
         $sizes[2] == $filesize && \
         $sizes[3] == $filesize && \
	  ...
	  ls ${CASENAME}.pop.h.${year}.nc
          if !( $status == 0 ) then
             ncra ${MSP}/${CASENAME}.pop.h.${year}-??.nc ${CASENAME}.pop.h.${year}.nc &
	     wait
	      ls ${MSPY}/${CASENAME}.pop.h.${year}.nc
	      if !( $status == 0 ) then
	       ls ${CASENAME}.pop.h.${year}.nc
	       if !(status == 0 ) then
                 mkdir -p $MSPY
		 mv ${CASENAME}.pop.h.${year}.nc ${MSPY}/${CASENAME}.pop.h.${year}.nc
		 ln -sf ${MSPY}/${CASENAME}.pop.h.${year}.nc ${CASENAME}.pop.h.${year}.nc 
               endif
	      endif
	    if ( $NEED_CLIM == 0 ) then 
	  ...
	  if ( $NEED_CLIM == 1 ) then
		 ...
		 if !( $status == 0 ) then
                   ncks -v $varstr ${MSP}/${CASENAME}.pop.h.${year}-0$mn.nc ${CASENAME}.pop.h.vars.${year}-0$mn.nc
		 endif
               else 
	         ls ${CASENAME}.pop.h.vars.${year}-$mn.nc
		 if !( $status == 0 ) then
                   ncks -v $varstr ${MSP}/${CASENAME}.pop.h.${year}-$mn.nc ${CASENAME}.pop.h.vars.${year}-$mn.nc
		 endif
               endif
             @ mn++
             end
...
wait
   ...
   if ( $NEED_CLIM == 1) then
     if ($yr0 == $yr1) then
      ncrcat -v ${varstr} ${MSP}/${CASENAME}.pop.h.vars.${year0}-??.nc ${CASENAME}.clim.${year0}-${year1}.nc
     else
